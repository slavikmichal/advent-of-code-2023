...
fun main() {
    ...
    val seedPairs = almanac[0][0].split(":")[1].trim().split("\\s+".toRegex()).map{ it.toLong() }.chunked(2)

    var locationMin: Long = Long.MAX_VALUE
    for (seedPair in seedPairs) {
        for (seed in seedPair[0]..(seedPair.sum() - 1)) {
